...
#' WriteCorpus
#'
#' \code{WriteCorpus} Encapsulates the command to write a Corpus or CVSet to file.
#'
...
#' @export
WriteCorpus <- R6::R6Class(
  classname = "WriteCorpus",
  lock_objects = FALSE,
  lock_class = FALSE,
  inherit = Cmd0,
  ...
  public = list(
    initialize = function(corpusStudio, path, overwrite = FALSE) {
      ...
      private$loadServices(name = 'WriteCorpus')
    ...
    execute = function(x) {
      ...
      corpus <- private$..corpusStudio$write(x,
                                            path = private$..path,
